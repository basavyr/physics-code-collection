...
class EllipticFunctions:
    ...
    def v_q_func_k(self, spin_values: list[float], theta_deg: float, q: float) -> np.ndarray:
        """
        ...
        for I in spin_values:
            ...
            k = np.sqrt(np.abs(u))
            k_squared = np.power(k, 2)
            # evaluates the Jacobi Amplitude using the modulus k
            phi_q = self.amu(q, k)
            s = np.sin(phi_q)
            s_squared = np.power(s, 2)
            c = np.cos(phi_q)
    ...
    def v_q_func_k_squared(self, spin_values: list[float], theta_deg: float, q: float) -> np.ndarray:
        """
        - Calculate the elliptic potential only from the angular momentum, the angle theta, and q coordinate
        - Works for a list of spin values
        - ! Uses the modulus m = k^2
        """
        v_values = np.array([])
        for I in spin_values:
            theta_rad = theta_deg*np.pi/180.0
            ...
            k = np.sqrt(np.abs(u))
            k_squared = np.power(k, 2)
            # evaluates the Jacobi Amplitude using the modulus k^2
            phi_q = self.amu(q, k_squared)
            s = np.sin(phi_q)
            s_squared = np.power(s, 2)
            c = np.cos(phi_q)
            d = np.sqrt(1.0-k_squared*s_squared)
